...
template <typename T>
void run_v4_benchmark(const char *tag, int M, int N, int K, int warmup, int iters)
{
    ...
    auto run_func = [&]()
    {
        if constexpr (std::is_same_v<T, double>)
            run_v4<double, double, double>(M, N, K, (double)alpha, (double)beta, dA, dB, dC, stream);
        else if constexpr (std::is_same_v<T, float>)
            run_v4<float, float, float>(M, N, K, alpha, beta, dA, dB, dC, stream);
        else if constexpr (std::is_same_v<T, half>)
            run_v4<half, float, float>(M, N, K, alpha, beta, dA, dB, dC, stream);
    };
    auto run_baseline = [&]()
    {
        if constexpr (std::is_same_v<T, double>)
...
int main(int argc, char **argv)
{
    ...
    run_v4_benchmark<half>("v4_fp16_mixed", M, N, K, warmup, iters);
    run_v4_benchmark<float>("v4_fp32", M, N, K, warmup, iters);
    run_v4_benchmark<double>("v4_fp64", M, N, K, warmup, iters);
