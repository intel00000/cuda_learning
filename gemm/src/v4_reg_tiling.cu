#include "v4_reg_tiling.h"
#include "baseline.h"

static void bench_v4_fp16(int M, int N, int K, int warmup, int iters, size_t ws_size)
{
    Bench<half, half> b(M, N, K, ws_size);
    float alpha = 1.25f, beta = 0.75f;
    b.run_pair("v4_fp16", warmup, iters, [&]()
               { run_v4<half, float, float>(M, N, K, alpha, beta, b.A(), b.B(), b.C(), b.stream()); }, [&]()
               { baseline_fp16_fp32acc_cm(b.lt(), b.stream(), M, N, K, alpha, beta, b.A(), b.B(), b.C_ref(), b.ws(), b.ws_size()); });
}
static void bench_v4_fp32(int M, int N, int K, int warmup, int iters, size_t ws_size)
{
    Bench<float, float> b(M, N, K, ws_size);
    float alpha = 1.25f, beta = 0.75f;
    b.run_pair("v4_fp32", warmup, iters, [&]()
               { run_v4<float, float, float>(M, N, K, alpha, beta, b.A(), b.B(), b.C(), b.stream()); }, [&]()
               { baseline_fp32_cm(b.lt(), b.stream(), M, N, K, alpha, beta, b.A(), b.B(), b.C_ref(), b.ws(), b.ws_size()); });
}
static void bench_v4_fp64(int M, int N, int K, int warmup, int iters, size_t ws_size)
{
    Bench<double, double> b(M, N, K, ws_size);
    double alpha = 1.25, beta = 0.75;
    b.run_pair("v4_fp64", warmup, iters, [&]()
               { run_v4<double, double, double>(M, N, K, alpha, beta, b.A(), b.B(), b.C(), b.stream()); }, [&]()
...
int main(int argc, char **argv)
{
    ...
    bench_v4_fp16(M, N, K, warmup, iters, ws_size);
    bench_v4_fp32(M, N, K, warmup, iters, ws_size);
    bench_v4_fp64(M, N, K, warmup, iters, ws_size);
    return 0;
}
