...
template <typename T>
void run_v1(const char *tag, int M, int N, int K, int warmup, int iters)
{
    ...
    T *dA = nullptr, *dB = nullptr, *dC = nullptr, *dC_ref = nullptr;
    ...
    auto run_func = [&]()
    {
        if constexpr (std::is_same_v<T, double>)
            run_v1<double, double, double>(M, N, K, (double)alpha, (double)beta, dA, dB, dC, stream);
        else if constexpr (std::is_same_v<T, float>)
            run_v1<float, float, float>(M, N, K, alpha, beta, dA, dB, dC, stream);
        else if constexpr (std::is_same_v<T, half>)
            run_v1<half, float, float>(M, N, K, alpha, beta, dA, dB, dC, stream);
    };
    auto run_baseline = [&]()
    {
        if constexpr (std::is_same_v<T, double>)
...
int main(int argc, char **argv)
{
    ...
    run_v1<half>("v1_fp16_mixed", M, N, K, warmup, iters);
    run_v1<float>("v1_fp32", M, N, K, warmup, iters);
    run_v1<double>("v1_fp64", M, N, K, warmup, iters);
