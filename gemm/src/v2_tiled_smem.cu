...
template <typename T>
void run_v2_benchmark(const char *tag, int M, int N, int K, int warmup, int iters)
{
    ...
    auto run_func = [&]()
    {
        if constexpr (std::is_same_v<T, double>)
            run_v2<double, double, double>(M, N, K, (double)alpha, (double)beta, dA, dB, dC, stream);
        else if constexpr (std::is_same_v<T, float>)
            run_v2<float, float, float>(M, N, K, alpha, beta, dA, dB, dC, stream);
        else if constexpr (std::is_same_v<T, half>)
            run_v2<half, float, float>(M, N, K, alpha, beta, dA, dB, dC, stream);
    };
    auto run_baseline = [&]()
    {
        if constexpr (std::is_same_v<T, double>)
...
int main(int argc, char **argv)
{
    ...
    run_v2_benchmark<half>("v2_fp16_mixed", M, N, K, warmup, iters);
    run_v2_benchmark<float>("v2_fp32", M, N, K, warmup, iters);
    run_v2_benchmark<double>("v2_fp64", M, N, K, warmup, iters);
