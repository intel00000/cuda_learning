...
template <typename T>
void run_v5_benchmark(const char *tag, int M, int N, int K, int warmup, int iters)
{
    ...
    auto run_func = [&]()
    {
        if constexpr (std::is_same_v<T, double>)
            run_v5<double, double, double>(M, N, K, (double)alpha, (double)beta, dA, dB, dC, stream);
        else if constexpr (std::is_same_v<T, float>)
            run_v5<float, float, float>(M, N, K, alpha, beta, dA, dB, dC, stream);
        else if constexpr (std::is_same_v<T, half>)
            run_v5<half, float, float>(M, N, K, alpha, beta, dA, dB, dC, stream);
    };
    auto run_baseline = [&]()
    {
        if constexpr (std::is_same_v<T, double>)
...
int main(int argc, char **argv)
{
    ...
    run_v5_benchmark<half>("v5_fp16_mixed", M, N, K, warmup, iters);
    run_v5_benchmark<float>("v5_fp32", M, N, K, warmup, iters);
    run_v5_benchmark<double>("v5_fp64", M, N, K, warmup, iters);
