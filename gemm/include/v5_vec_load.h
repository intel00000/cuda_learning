...
#pragma unroll
        for (int i = 0; i < A_vecs_per_thread; i++)
        {
            ...
            if (vec_idx < A_total_vecs)
            {
                ...
                if (global_row + VEC - 1 < M && global_col < K)
                {
                    // Vectorized load, cast global memory to vector type
                    VecT tmp = *reinterpret_cast<const VecT *>(&A[global_row + global_col * M]);
#pragma unroll
                    // Store transposed to shared memory, compiler autovectorizes
                    for (int v = 0; v < VEC; v++)
                        As[col][row_base + v] = static_cast<AccT>(tmp.data[v]);
                }
...
#pragma unroll
                    for (int v = 0; v < VEC; v++)
                    {
                        int r = global_row + v;
                        if (r < M && global_col < K)
                            As[col][row_base + v] = static_cast<AccT>(A[r + global_col * M]);
                        else
                            As[col][row_base + v] = AccT(0);
                    }
                }
            }
        }
...
#pragma unroll
        for (int i = 0; i < B_vecs_per_thread; i++)
        {
            ...
            if (vec_idx < B_total_vecs)
            {
                ...
                if (global_k + VEC - 1 < K && global_n < N)
                {
                    // Vectorized load, cast global memory to vector type
                    VecT tmp = *reinterpret_cast<const VecT *>(&B[global_k + global_n * K]);
#pragma unroll
                    // Store transposed to shared memory, compiler autovectorizes
                    for (int v = 0; v < VEC; v++)
                        Bs[col][row_base + v] = static_cast<AccT>(tmp.data[v]);
                }
                else // Scalar fallback
                {
#pragma unroll
                    for (int v = 0; v < VEC; v++)
                    {
                        int k = global_k + v;
                        if (k < K && global_n < N)
                            Bs[col][row_base + v] = static_cast<AccT>(B[k + global_n * K]);
                        else
                            Bs[col][row_base + v] = AccT(0);
                    }
                }
            }
        }
        ...
        for (int i = 0; i < TM; i += VEC)
        {
            ...
            else // Scalar fallback
            {
                for (int v = 0; v < VEC; v++)
                {
                    ...
                    if (r < M)
                    {
                        AccT c_val = static_cast<AccT>(C[r + global_col * M]);
                        C[r + global_col * M] = static_cast<T>(a_scale * acc[i + v][j] + b_scale * c_val);
                    }
                }
            }
        }
...
template void run_v5<half, half, half>(int, int, int, half, half, const half *, const half *, half *, cudaStream_t);
